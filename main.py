...
def get_all_products_from_dropshipping(persist=False):
    ...
    all_items = dropshipping_filter.remove_watches_by_reference(all_items)
...
def get_all_watches_from_shopify(persist):
    ...
    if persist:
        write_to_file("shopify_dropshipping_products.json", result, False)

    logger.info(f"{len(result)} products retrieved from shopify")
    return result


def update_quantity_if_differ(
    ...
    for shopify_item in shopify_products:
        dropshipping_product_id = get_dropshipping_id_from_shopify_product(shopify_item)
        result = list(
            filter(
                lambda x: x.id_product == dropshipping_product_id, dropshipping_products
            )
        )
        if len(result) > 0:
            ...
            if shopify_quantity is not dropshipping_quantity:
                ...
                shopify.set_inventory_of_product(inventory_item_id, result[0].stock)
...
def update_prices_of_products_if_differ(
    ...
    for item in shopify_products:
        ...
        if len(result):
            ...
            shopify_retail_price = (
                float(item["variants"][0]["compare_at_price"])
                if item["variants"][0]["compare_at_price"] is not None
                else "null"
            )
            ...
            if dropshipping_price != shopify_price:
                logger.info(
                    ...
                    + item["title"]
                    + " from: "
                    + str(item["variants"][0]["price"])
                    + " to "
                    + str(dropshipping_price)
                )
            ...
            if dropshipping_retail_price != shopify_retail_price:
                logger.info(
                    "update retail price of "
                    + item["title"]
                    + " from: "
                    + str(item["variants"][0]["compare_at_price"])
                    + " to "
                    + str(dropshipping_retail_price)
                )
    ...
    # item["new_price"] = dp_item.get_selling_price() # only for testing! add this if price_comparison method should be called
    # price_comparison(shopify_products, 'price_comparison') # only for testing! create file to compare price difference


def add_product_if_not_present(
    dropshipping_products: List[DropshippingItem], shopify_products
):
    ...
    for dropshipping_item in dropshipping_products:
        result = list(
            filter(
                lambda x: dropshipping_item.id_product
                == get_dropshipping_id_from_shopify_product(x),
                shopify_products,
            )
        )
        if len(result) == 0:
            ...
            shopify.set_inventory_of_product(inventory_item_id, dropshipping_item.stock)

            # add product to collection ("dropshipping" category)
            shopify.add_product_to_collect(response["product"]["id"])
...
def main():
    try:
        ...
        update_prices_of_products_if_differ(dropshipping_products, shopify_products)
